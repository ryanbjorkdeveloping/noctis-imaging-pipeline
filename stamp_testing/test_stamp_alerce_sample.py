...
# stamp_testing/ sits one level under the project root; add the root to the
# import path so `from ztf_classification...` etc. resolve when run from anywhere.
...
# ── Test: build both ingredients AND run the decision rule for one object ──────
client = Alerce()

oid = "ZTF17aaaacet"                    # a confident asteroid from Step 3 (live)
cube = fetch_cube(client, oid)

print(f"object:      {oid}")
print(f"cube shape:  {cube.shape}")           # expect (21, 21, 3)
print(f"cube dtype:  {cube.dtype}")           # expect float32
print(f"any NaNs?    {np.isnan(cube).any()}") # expect False
print(f"min / max:   {cube.min():.3f} / {cube.max():.3f}")

for i, name in enumerate(["science", "reference", "difference"]):
    ch = cube[:, :, i]
    print(f"  {name:11s} mean={ch.mean():9.3f}  peak={ch.max():9.3f}")

# Second ingredient: the metadata vector.
meta = assemble_metadata_vector(client, oid)
print(f"\nmetadata vector ({len(meta)} fields):")
for field, value in zip(META_FIELDS, meta):
    print(f"  {field:11s} = {value:.4f}")
print(f"  dtype = {meta.dtype}, any NaN? {np.isnan(meta).any()}")

# ── A3: the decision rule — 5-class softmax → final astrophysical verdict ──────
probs = fetch_stamp_probabilities(client, oid)
top_class, top_prob, astro = classify_astrophysical(probs)

print(f"\n5-class softmax:  {probs}")
print("after dropping bogus + renormalizing:")
for cls in ASTRO_CLASSES:
    print(f"  {cls:9s} {astro[cls]:.3f}")
print(f"→ VERDICT: {top_class} ({top_prob:.3f})")
